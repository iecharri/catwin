...
<span class='verde b'>Cuentas5</span>&nbsp; &nbsp; 
...
$rs=$link->query("SELECT cuenta, descripcio, sdo5d, sdo5h FROM cuentas5 WHERE sdo5d != 0 || sdo5h != 0 ORDER BY descripcio, cuenta");
...
while($row=$rs->fetch_array(MYSQLI_BOTH)) {
	...
	if ($row["sdo5d"] == 0){
		...
		echo number_format($row["sdo5d"]*$_SESSION['moneda'],$_SESSION['deci'],',','.');
	}
	...
	if ($row["sdo5h"] == 0){
		...
		echo number_format($row["sdo5h"]*$_SESSION['moneda'],$_SESSION['deci'],',','.');
	}
...
$sql="SELECT SUM(sdo5d) AS tot_saldod, SUM(sdo5h) AS tot_saldoa FROM cuentas5"; 
