...
<span class='verde b'>Cuentas</span>&nbsp; &nbsp; 
...
$rs=$link->query("SELECT cuenta, descripcio, sdo3cd, sdo3ca FROM cuentas WHERE sdo3cd != 0 || sdo3ca != 0 ORDER BY descripcio");
...
while($row=$rs->fetch_array(MYSQLI_BOTH)) {
	...
	if ($row["sdo3cd"] == 0){
		...
		echo number_format($row["sdo3cd"]*$_SESSION['moneda'],$_SESSION['deci'],',','.');
	}
	...
	if ($row["sdo3ca"] == 0){
		...
		echo number_format($row["sdo3ca"]*$_SESSION['moneda'],$_SESSION['deci'],',','.');
	}
...
$sql="SELECT SUM(sdo3cd) AS tot_saldod, SUM(sdo3ca) AS tot_saldoa FROM cuentas"; 
